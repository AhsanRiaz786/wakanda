...
curl -sf -X POST "$BASE/simulate" -H "Content-Type: application/json" \
  -d "{\"planId\":\"$PLAN_ID\",\"overrides\":{\"forceApiFailure\":true}}" | head -c 200 || true
echo ""

echo "==> Trace"
curl -sf "$BASE/trace?planId=$PLAN_ID" | head -c 200 || true
echo ""
echo "Smoke test OK"
