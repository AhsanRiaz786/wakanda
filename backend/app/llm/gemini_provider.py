...
class GeminiLLMProvider:
    """Gemini LLM implementation for city agent APIs."""
    ...
    def resolve_contradiction(self, incidents: list[Incident]) -> ResolvedConflict:
        # Pre-select winner using algorithm (Task 2)
        # Note: The agent brief says "Winner selection is deterministic (contradiction.resolve_group)."
        # We'll import it locally to avoid circular dependencies if needed, or we just
        # construct a prompt that asks the LLM to verify the winner and generate rationale.
