...
def validate_input(body: IngestRequest) -> dict | None:
    if len(body.rawDescription.strip()) < 10:
        return {"code": "DESCRIPTION_REQUIRED", "message": "Description must be at least 10 characters"}
    return None
...
def normalize_location(store: WorkspaceStore, body: IngestRequest) -> tuple[Coordinates | None, bool, str | None, float]:
    ...
    for existing in store.incidents:
        if existing.description == incident.description:
            if existing.coordinates and incident.coordinates:
                dist = abs(existing.coordinates.lat - incident.coordinates.lat) + abs(existing.coordinates.lng - incident.coordinates.lng)
...
def run_ingest(store: WorkspaceStore, body: IngestRequest) -> tuple[Incident | None, list[dict], dict | None, bool]:
    trace: list[dict] = []
    ...
    norm_ts, raw_ts = normalize_timestamp(body.rawTimestamp if hasattr(body, 'rawTimestamp') else None)
