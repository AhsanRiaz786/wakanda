...
from app.tools.contradiction import detect_contradiction_groups
...
def resolve_contradictions(
    incidents: list[Incident], groups: list[ContradictionGroup]
) -> dict[str, dict]:
    resolutions = {}
...
def route_and_match(
    store: WorkspaceStore, incidents: list[Incident], classifications: dict[str, dict]
) -> dict[str, dict]:
    results = {}
...
def build_chains(
    incidents: list[Incident], classifications: dict[str, dict], routing_results: dict[str, dict]
) -> dict[str, list[dict]]:
    chains = {}
    for inc in incidents:
        cls = classifications.get(inc.incidentId)
        ...
        if itype in (IncidentType.WATER_LEAK, IncidentType.ROAD_BLOCKAGE):
            chain.append(
                {
                    "step": 4,
                    "type": ActionStepType.MANAGE_ROAD_IMPACT.value,
                    "status": "pending",
                    "roadIds": ["RD-02"],
                }
            )
        chain.append(
            {
                "step": 5,
                "type": ActionStepType.SCHEDULE_FOLLOWUP.value,
                "status": "pending",
                "scheduledAtMinutes": 90,
            }
        )
...
def check_constraints(
    routing_results: dict[str, dict], constraints: PlanConstraints | None
...
def draft_all_notifications(
    incidents: list[Incident], classifications: dict[str, dict]
) -> dict[str, dict]:
    drafts = {}
    for inc in incidents:
        cls = classifications.get(inc.incidentId)
        if cls:
            drafts[inc.incidentId] = llm_tools.draft_notifications(
                inc.incidentId, cls["incidentType"].value
            )
    return drafts
    ...
    if body.planMode != "quick":
        ...
        if groups:
            ...
            append_step(
                ...
                output_summary=f"Resolved {len(groups)} conflicts",
                decision_rationale=resolutions[groups[0].incident_ids[0]]["rationale"]
                if groups
                else None,
            )
