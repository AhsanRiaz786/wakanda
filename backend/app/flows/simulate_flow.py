...
def build_animation_frames(before: CityState, after: CityState, actions: list[SimulatedAction], speed: str) -> list[dict]:
...
def persist_simulation(store: WorkspaceStore, plan_id: str, started_at: str, before: CityState, after: CityState, actions: list[SimulatedAction], failures: list[FailureRecord], metrics: dict, frames: list[dict]) -> SimulationRun:
    run = SimulationRun(
...
def run_simulate(store: WorkspaceStore, body: SimulateRequest) -> tuple[SimulationRun, list[dict]]:
    ...
    append_step(
        trace, step_id="SIM01", name="Load plan", step_type=TraceStepType.STATE_UPDATE,
    ...
    append_step(
        trace, step_id="SIM02", name="Execute chains", step_type=TraceStepType.STATE_UPDATE,
    ...
    if failures:
        append_step(
            trace, step_id="SIM-FAIL", name="Simulated API failure", step_type=TraceStepType.ERROR,
            status=TraceStepStatus.WARNING, output_summary="NOTIFICATION_API_TIMEOUT — retried successfully"
        )
