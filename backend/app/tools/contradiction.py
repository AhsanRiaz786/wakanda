...
def detect_contradiction_groups(incidents: list["Incident"]) -> list[ContradictionGroup]:
    ...
    for i in range(len(sorted_inc)):
        ...
        for j in range(i + 1, len(sorted_inc)):
            ...
            dist_m = haversine_m(
                inc1.coordinates.lat, inc1.coordinates.lng,
                inc2.coordinates.lat, inc2.coordinates.lng
            )
            ...
            for a in cluster:
                for b in cluster:
                    if a.incidentId != b.incidentId and a.coordinates and b.coordinates:
                        d = haversine_m(a.coordinates.lat, a.coordinates.lng, b.coordinates.lat, b.coordinates.lng)
